...
export default function DataTable() {
    ...
    useEffect(() => {
        const id = setInterval(() => fetchData(date || undefined), 6000);
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            await Promise.all(
                selectedIds.map(id => fetch(`${API}/data/${id}`, { method: "DELETE" }))
            );
    ...
    return (
        <div className="p-4">
            <div className="flex flex-wrap items-center gap-4 mb-4">
                ...
                <button
                    className={`px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 transition ${selectedIds.length === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
                        ...
                        {items.length === 0 ? (
                            ...
                            items.map((it, idx) => {
                                ...
                                return (
                                    <tr
                                        key={it._id || idx}
                                        className={`hover:bg-gray-50 transition ${isSelected ? "bg-blue-100" : ""}`}
                                    >
                                        ...
                                        <td className="px-6 py-2 border-b">{p.temperature ?? "-"}</td>
                                        <td className="px-6 py-2 border-b">
                                            <button
